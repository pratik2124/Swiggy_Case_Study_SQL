...
SELECT DISTINCT(restaurant_name), (COUNT(case when veg_or_nonveg = 'Veg' then 1 end) * 100/count(*)) as vegetarian_percentage
from swiggy
...
SELECT DISTINCT(restaurant_name), (count(case when veg_or_nonveg = 'Non-Veg' then 1 end) *100 /count(*)) as nonvegetarian_percentage 
from swiggy 
group by restaurant_name 
